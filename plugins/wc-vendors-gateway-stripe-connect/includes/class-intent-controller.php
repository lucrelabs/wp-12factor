...
class WCV_SC_Intent_Controller {
	...
	protected function get_order_from_request() {
		if ( ! isset( $_GET['nonce'] ) || ! wp_verify_nonce( sanitize_key( $_GET['nonce'] ), 'wc_stripe_confirm_pi' ) ) {
			throw new WCV_SC_Exception( 'missing-nonce', __( 'CSRF verification failed.', 'wc-vendors-stripe-connect' ) );
		}
		...
		if ( ! $order ) {
			throw new WCV_SC_Exception( 'missing-order', __( 'Missing order ID for payment confirmation', 'wc-vendors-stripe-connect' ) );
		}
