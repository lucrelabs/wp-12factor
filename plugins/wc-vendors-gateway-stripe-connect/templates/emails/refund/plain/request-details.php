...
echo wp_kses_post( wc_strtoupper( sprintf( __( '[Request #%1$s] (%2$s)', 'wc-vendors-stripe-connect' ), $request->get_id(), wc_format_datetime( $request->get_date_created() ) ) ) ) . "\n";
...
if ( $request->get_reason() ) {
	echo esc_html__( 'Reason:', 'wc-vendors-stripe-connect' ) . "\t " . wp_kses_post( wptexturize( $request->get_reason() ) ) . "\n";
}

if ( $sent_to_admin ) {
	/* translators: %s: Order link. */
	echo "\n" . sprintf( esc_html__( 'View request: %s', 'wc-vendors-stripe-connect' ), esc_url( $request->get_edit_url() ) ) . "\n";
}
